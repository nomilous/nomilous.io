...
module.exports = function(id, hostname, port) {
  ...
  var Promise, THREE, aspect, camera, canvas, client, container, dom, far, fov, height, near, radianRatio, radius, renderer, scene, toLatitude, toLongitude, transform, uplink, width, xhr;
  dom = require('dom');
  THREE = require('three');
  xhr = require('xhr');
  ...
  uplink.connect();
  uplink.socket.on('message', function(payload) {
    return console.log({
      received: payload
    });
  });
  ...
  return Promise.all(['/earth', '/visitors'].map(function(path) {
    ...
    var animate, city, country, earth, geometry, landMasses, ll, material, me, particles, polygon, region, vertex, visitors, _i, _j, _k, _len, _len1, _len2, _ref;
